...
```bash
pnpm add zh-kit          # pnpm（推荐）
npm install zh-kit       # npm
yarn add zh-kit          # yarn
```
...
```ts
// 按需导入（推荐）
import { formatDate, uploadFile, isPhone, formatMoney } from 'zh-kit'
```
...
```ts
import { isPhone, isEmail, isIdCard, isUrl, isLicensePlate, isEmpty, isChinese, isDecimal } from 'zh-kit'
...
```ts
import { deepClone, deepMerge, omit, pick, clearObj, isPlainObject, get, set, flatten, diff, merge } from 'zh-kit'
...
```ts
import { formatMoney, formatPercent, formatMobile, formatIdCard, formatBankCard, formatFileSize } from 'zh-kit'
...
```ts
import { getUrlParam, setUrlParam, removeUrlParam, objToUrlParams, urlParamsToObj } from 'zh-kit'
...
```ts
import { formatDate, formatDateTime, formatRelative, getStartOfDay, getEndOfDay, getDateRange, diffDays, getAge, addDays, addMonths, isBefore, isAfter, isSameDay } from 'zh-kit'
...
```ts
import { listToTree, treeToList, findTreeNode, findTreePath, filterTree, walkTree, mapTree } from 'zh-kit'
...
```ts
import { getLocal, setLocal, removeLocal, clearLocal, getSession, setSession, getCookie, setCookie, removeCookie } from 'zh-kit'
...
```ts
import { hasPermission, hasRole, hasAllPermissions, hasAnyPermission } from 'zh-kit'
...
```ts
import { downloadByUrl, downloadByBlob, uploadFile, getFileSizeStr, getFileExtension, isFileType, readFileAsDataURL, readFileAsText, download } from 'zh-kit'
...
```ts
import { compressImage, imageToBase64, getImageDimensions, createImageThumbnail } from 'zh-kit'
...
```ts
import { debounce, throttle } from 'zh-kit'
...
```ts
import { copyToClipboard } from 'zh-kit'
...
```ts
import { uuid, randomStr, camelToSnake, snakeToCamel, truncate, escapeHtml, stripHtml } from 'zh-kit'
...
```ts
import { sleep, once, retry } from 'zh-kit'
...
```ts
import { groupBy, unique, uniqueBy, chunk, range, sortBy, sumBy, move } from 'zh-kit'
...
```ts
import { add, sub, mul, div, round, clamp, random, inRange } from 'zh-kit'
